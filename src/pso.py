...
def pso(func, lb, ub, ieqcons=[], f_ieqcons=None, args=(), kwargs={}, 
        ...
        minstep=1e-8, minfunc=1e-8, debug=False, load_file=False, g0=[]):
    """
    ...
    debug : boolean
        ...
        (Default: False)
    g0 : list 
        A guess for the swarm's best known position
    ...
    def is_feasible(x):
        check = np.all(np.asarray(cons(x))>0)
        return check
    ...
    for i in range(S):
        ...
        if len(g) == 0 and is_feasible(x[i, :]):
            g = p[0, :].copy()
    ...
    # If best swarm position is empty, give position upper boundary values
    
    if g == []:
        print("Couldn't find feasible starting point. Try again")
        return g, fg
