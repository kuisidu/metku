...
for beam_size in ['IPE 270']:#, 'IPE 400', 'IPE 500', 'IPE 600']:
    ...
    margin = 0.05
    S_j_limit = 16000.0*1.0e6

    method='SLSQP'

    file_name = 'Test_Diaz_C_0,05_c_' + str(cn.column.size) + '_b_' + str(beam_size)
    ...
    #for sd.d in bolt_sizes:
        cn.add_end_plate(beam_id=0, b=b, t=15, upper_overhang=1000.0, material="S275", equ=True)
        sd.e = 1.2*sd.d
        for k in range(0, 1):
            start = timer()
            run += 1
