...
class SemiRigidFrame:
    ...
    def add_materials_and_sections(self):
        ...
        for key in self.members.keys():
            ...
            if member.profile != 'Custom':
                member.cross_section_properties()
            member.add_material()
            member.add_section()
    ...
    def draw_frame(self, print_text=True):
        """ Draws the frame
    ...
    def draw_deflected_frame(self, scale=1, prec=4):
        ...
        """
        self.draw_frame(print_text=False)
    ...
    def find_closest_profile(self, AI):
    ...
    def continuous_to_discrete(self):
        ...
        for key in self.members.keys():
            member = self.members[key]
            AI = [member.A, member.I_y]
            member.profile = self.find_closest_profile(AI)        
        self.generate_frame()
        self.calculate()
    ...
    def costf_weight(self, X=[]):
        ...
        rho = 7850e-9
        # IPE values
        K = [0.0155, 2.4921, 0.0256, 3.2831]
        # HEA values
        # K = [0.0195, 2.9158, 0.0405, 2.9034]
        # HEB values
        # K = [0.0220, 4.3311, 0.0453, 5.7595]
        weight = 0
        index = 0
        # weight of the beams
    ...
    def func(self, x):
        """ Fuction, that assigns x's-values to frame's members
        ...
        for _ in range(math.floor(self.bays/2)):
            for i in range(self.storeys):
                ...
                member1.custom_section_properties([x[index], x[index+1]])
                member2.custom_section_properties([x[index], x[index+1]])                
        ...
        if self.bays % 2 != 0:
            for i in range(self.storeys):
                ...
                member.custom_section_properties([x[index], x[index+1]])                
                # Symmetrical joint rigidities
    ...
    def optimize_frame(self, debug=True, print_data=False, optimizer="",
                       ...
                       phig=0.5, penalty_val=1000, joints=True, log_data=False,
...
class FrameMember:
    ...
    def custom_section_properties(self, x):
        ...
        self.b = x[1]
        if len(x) == 2:
            self.t_w = K[0]*self.h + K[1]
            self.t_f = K[2]*self.h + K[3]
            self.round = 5
        elif len(x) == 5:
            self.t_w = x[2]
            self.t_f = x[3]
    ...
    def add_section(self):
        ...
        s = fem.BeamSection(self.A*1e-6, self.I_y*1e-12)
    ...
    def check_cross_section(self):
        ...
        self.r = []
        cross_section = CrossSection(self.f_y,self.h, self.b, self.t_f,
                                     self.A,[self.I_y, self.I_z],0,
                                     [self.W_pl_y,self.W_pl_z],
                                     [self.W_el_y,self.W_el_z],self.I_w,
                                     self.I_t, self.A_v,
                                     Ned=self.ned,Med=self.med,Ved=self.ved)
        steel_member = SteelMember(cross_section,self.length,
