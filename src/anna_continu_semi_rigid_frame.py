...
class SemiRigidFrame:
        ...
        for key in self.members.keys():
            ...
            for i in range(len(member.nodes)):
                ...
                if member.mtype == "beam":
                    ...
                    y1 = member.nodal_forces[node][2]/(1/scale)                      
                    x = x0
                    y = y0 - y1                    
                    X.append(x)
                    Y.append(y)                    
                if member.mtype == "column":
                    ...
                    x1 = member.nodal_forces[node][2]/(1/scale)                     
                    x = x0 + x1
                    y = y0                    
                    X.append(x)
                    Y.append(y)
    ...
    def costf_weight(self, X=[]):
        ...
        """
            ...
            weight += A * rho * self.storey_height # kg's
...
class FrameMember:
    ...
    def alpha_to_Sj(self):
        ...
        self.Sj1 = self.alpha1 * self.E * self.cross_section.I[0] / self.length
        self.Sj2 = self.alpha2 * self.E * self.cross_section.I[0]/ self.length
    ...
    def Sj_to_alpha(self):
        ...
        self.alpha1 = self.Sj1 * self.length / (self.E * self.I_y)
        self.alpha2 = self.Sj2 * self.length / (self.E * self.I_y)
    ...
    def check_steel_section_HEA(self):
        ...
        for node in self.nodal_forces.keys():
            ...
            #ved = forces[1] *1e3        # kN to N
            #med = forces[2] *1e6        # kNm to Nmm
            ned = forces[0]       # kN to N
            ved = forces[1]         # kN to N
            med = forces[2]         # kNm to Nmm
