...
sys.path.append('S:\\91202_METKU\\Kristo\\Python\\frame_2d_git\\src\\End-plate')
import math
...
def cross_section_properties(b,h,tf,tw,r):
    ...
    I = [0.0,0.0]
    I[0] = 1.0/12.0*(b*h**3.0 - (b - tw)*(h - 2.0*tf)**3.0) + 0.03*r**4 + 0.2146*r**2.0*(h - 2.0*tf - 0.4468*r)**2.0
    I[1] = 1.0/12.0*(2.0*tf*b**3.0 + (h - 2.0*tf)*tw**3.0) + 0.03*r**4.0 + 0.2146*r**2.0*(tw + 0.4468*r)**2.0
    ...
    Wpl = [0.0,0.0]
    Wpl[0] = (tw*h**2.0)/4.0 + (b - tw)*(h - tf)*tf + (4.0 - math.pi)/2.0*r**2.0*(h - 2.0*tf) + (3.0*math.pi - 10.0)/3.0*r**3.0
    Wpl[1] = (b**2.0*tf)/2.0 + (h - 2.0*tf)/4.0*tw**2.0 + r**3.0*(10.0/3.0 - math.pi) + (2.0 - math.pi/2.0)*tw*r**2.0
