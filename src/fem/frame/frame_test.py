...
for j in range(2):
    for i in range(storeys+1):
        ...
        if i > 0:
            el = EBBeam(f.nodes[j*(storeys+1)+i-1],f.nodes[j*(storeys+1)+i],s,f.materials[0])  
            f.add_element(el)
...
for i in range(nbeam_els-1):
    for j in range(storeys):
        ...
        if i == 0:
            if sr_frame:
                el = EBSemiRigidBeam(f.nodes[N-DN],f.nodes[N],s,f.materials[0],rot_stiff=[Sj1,Sj2])            
            else:
                el = EBSemiRigidBeam(f.nodes[N-DN],f.nodes[N],s,f.materials[0])            
        elif i == nbeam_els-2:
            el = EBBeam(f.nodes[N-storeys],f.nodes[N],s,f.materials[0])
            f.add_element(el)
            if line_load:
                load = frame_fem.LineLoad(2,el,[0,1],-5*np.array([1,1]),1)
                f.add_load(load)
            if sr_frame:
                el = EBSemiRigidBeam(f.nodes[N],f.nodes[storeys+2+j],s,f.materials[0],rot_stiff=[Sj2,Sj1])            
            else:
                el = EBBeam(f.nodes[N],f.nodes[storeys+2+j],s,f.materials[0])
        else:
            el = EBBeam(f.nodes[N-storeys],f.nodes[N],s,f.materials[0])
