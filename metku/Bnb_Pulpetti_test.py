...
from sections.timber.timber_section import TimberSection, PulpettiPalkki, HarjaPalkki
...
class Keha(sop.OptimizationProblem):
    ...
    def luo_muuttujat(self):
        ...
        palk_h = sop.Variable(name=f'Palkki H0', lb=100, ub=1000,
                             target={'property': 'H', 'objects': [self.structure.members[1].cross_section]})
        # palk_hap = sop.Variable(name=f'PulpettiPalkki Hap', lb=100, ub=10000,
...
if __name__ == '__main__':
    ...
    nlp_solver = SLSQP()
    solver = BnB(ongelma, lb_solver=nlp_solver)
    #solver = SLP()
    fopt, xopt = solver.solve(ongelma, x0=x0, maxiter=10, verb=False)
