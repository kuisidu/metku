...
class Truss2D(Frame2D):
    ...
    def get_deflection(self, load_id: LoadIDs = None) -> float:
        """
        Returns maximum deflection of bottom chords
        :param load_id: load id
        :return: float, maximum displacement
        """
        ...
        if load_id is None:
            try:
                ...
                raise Exception(
                    "!!! You need to add loads and calculate model "
                    "before trying to get maximum deflection !!! ")
        if load_id not in self.load_ids:
            raise ValueError(f"No loads with ID:{load_id} in the current model")
        abs_max = 0
        for bc in self.bottom_chords:
            disps = bc.nodal_displacements[load_id]
            # disp = [dx, dy, rz]
            abs_Y = [abs(disp[1]) for disp in disps.values()]
...
class TrussMember(FrameMember):
    ...
    def plot(self, print_text=True, color=True, axes=None):
        ...
        if delta_x == 0:
            ...
            rot = np.degrees(np.arctan(delta_y / delta_x))
            # rot = self.angle
...
class TrussJoint:
    ...
    def N0(self, load_id: LoadIDs = LoadIDs.ULS):
        """
        ...
        if self.joint_type == 'Y':
            N0 = self.chord.nodal_forces[load_id][self.nodes['c0'].nid][0]


        elif self.joint_type == 'K':
            N01 = self.chord.nodal_forces[load_id][self.nodes['c01'].nid][0]
            N02 = self.chord.nodal_forces[load_id][self.nodes['c02'].nid][0]
    ...
    def V0(self, load_id: LoadIDs = LoadIDs.ULS):
        """
        Returns maximum shear force on chord on gap

        :return: max N
        """
        V0 = self.chord.nodal_forces[load_id][self.nodes['c0'].nid][1]
        return abs(V0)

        # V = [abs(elem.axial_force[0]) for elem in self.elements.values()]
        # return max(V)

    def M0(self, load_id: LoadIDs = LoadIDs.ULS):
        if self.joint_type == 'Y':
            M0 = self.chord.nodal_forces[load_id][self.nodes['c0'].nid][2]

        elif self.joint_type == 'K':
            M01 = self.chord.nodal_forces[load_id][self.nodes['c01'].nid][2]
            M02 = self.chord.nodal_forces[load_id][self.nodes['c02'].nid][2]
    ...
    def update_forces(self, load_id=LoadIDs.ULS):
        """
        Updates RHS_joint's forces
        """
        self.rhs_joint.M0 = self.M0(load_id)
        self.rhs_joint.N0 = self.N0(load_id)
        self.rhs_joint.V0 = self.V0(load_id)
